import { db } from './db';
import { InsertRentalOption, rentalOptions } from '@shared/schema';
...
async function addRentalOptions() {
  try {
    console.log(`Iniziando l'aggiunta di opzioni di noleggio per i veicoli da ID ${START_ID} a ${END_ID}...`);
    ...
    console.log(`Trovati ${vehicles.length} veicoli nel range specificato`);
    
    // Aggiungi opzioni di noleggio per ogni veicolo
    for (const vehicle of vehicles) {
      console.log(`Processando veicolo ID ${vehicle.id}: ${vehicle.title}`);
      ...
      if (contractType === 'NLT' || contractType === 'BOTH') {
        await addNLTOptions(vehicle.id, vehicle);
        console.log(`  Aggiunte opzioni NLT`);
      }
      
      if (contractType === 'RTB' || contractType === 'BOTH') {
        await addRTBOptions(vehicle.id, vehicle);
        console.log(`  Aggiunte opzioni RTB`);
      }
    }
